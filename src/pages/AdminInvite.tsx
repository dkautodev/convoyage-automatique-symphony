...
const inviteFormSchema = z.object({
  ...
  expirationDays: z.coerce.number().int().min(1, {
    message: "Minimum 1 jour"
  }).max(30, {
    message: "Maximum 30 jours"
  })
});
...
export default function AdminInvite() {
  ...
  console.log("Chargement de la page d'invitation admin", {
    user,
    profile
  });
  ...
  const fetchTokens = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('admin_invitation_tokens').select('*').order('created_at', {
        ascending: false
      });
  ...
  const onSubmit = async (data: InviteFormValues) => {
    ...
    try {
      ...
      const {
        data: existingToken
      } = await supabase.from('admin_invitation_tokens').select('*').eq('email', normalizedEmail).eq('used', false).gt('expires_at', new Date().toISOString()).maybeSingle();
      if (existingToken) {
        console.warn("Un token actif existe déjà pour cet email:", existingToken);
        const confirmOverwrite = window.confirm(`Un token actif existe déjà pour ${normalizedEmail}. Voulez-vous créer un nouveau token qui remplacera l'ancien ?`);
        if (!confirmOverwrite) {
        ...
        const {
          error: updateError
        } = await supabase.from('admin_invitation_tokens').update({
          used: true,
      ...
      const {
        error: insertError
      } = await supabase.from('admin_invitation_tokens').insert({
        email: normalizedEmail,
        token: token,
        expires_at: expiresAt.toISOString(),
        created_by: user.id
