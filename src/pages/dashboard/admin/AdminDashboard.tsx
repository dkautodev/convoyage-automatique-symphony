                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge className={missionStatusColors[mission.status] || 'bg-gray-500'}>
                          {missionStatusLabels[mission.status] || 'Inconnu'}
                        </Badge>
                      </div>
                      <p className="text-sm text-gray-600">
                        {mission.pickup_address?.city || 'Départ'} → {mission.delivery_address?.city || 'Arrivée'} · {mission.distance_km?.toFixed(2) || '0'} km
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        Client: {mission.profiles?.company_name || mission.profiles?.full_name || 'Client inconnu'} · 
                        {mission.price_ttc ? mission.price_ttc.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' }) : '0 €'}
                      </p>
                    </div>
                    <Button variant="outline" size="sm" asChild>
