...
import { missionStatusColors, missionStatusLabels, MissionFromDB, convertMissionFromDB, MissionStatus } from '@/types/supabase';
...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const statusPromises = statuses.map(status => typedSupabase.from('missions').select('*', {
        count: 'exact'
      }).eq('status', status).then(({
        count
      }) => ({
        status,
        count: count || 0
      ...
      const {
        data: yearMissions
      } = await typedSupabase.from('missions').select('price_ht, price_ttc, chauffeur_price_ht, chauffeur_id, created_at, status').gte('created_at', `${currentYear}-01-01`).lte('created_at', `${currentYear}-12-31`);
      let totalRevenue = 0;
      let totalVat = 0;
      ...
      } = await typedSupabase.from('missions').select('*, profiles(full_name, company_name, email)').order('created_at', {
        ascending: false
      }).limit(5);
      setStats({
  ...
  return <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex space-x-2">
          <Button onClick={handleCreateNewMission} className="sm:bg-primary sm:text-primary-foreground sm:hover:bg-primary/90 sm:w-auto w-full bg-neutral-900 text-white hover:bg-neutral-800 h-10 text-xs rounded-sm font-medium py-0 mx-0 my-[10px]">
        ...
        <CardContent>
          {loading ? <div className="flex justify-center py-8">
              ...
              <Table>
                ...
                <TableBody>
                  {Object.entries(statusCounts).map(([status, count]) => <TableRow key={status}>
      ...
      <Card className="bg-white">
        ...
        <CardContent>
          {loading ? <div className="flex justify-center py-20">
              <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-admin"></div>
            </div> : <div className={isMobile ? "h-60" : "h-80"}>
              <Suspense fallback={<div className="flex justify-center items-center h-full">
                  <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-admin"></div>
                </div>}>
                <ResponsiveContainer width="100%" height="100%">
                  <RechartsBarChart data={monthlyRevenue} margin={{
                top: 20,
      ...
      <Card className="bg-white">
        ...
        <CardContent>
          {loading ? <div className="flex justify-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-admin"></div>
            </div> : recentMissions.length === 0 ? <div className="text-center py-8 text-gray-500">
              ...
              <Table>
                ...
                return <TableRow key={mission.id}>
