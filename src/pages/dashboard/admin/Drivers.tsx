...
const DriversPage = () => {
  ...
  const fetchDrivers = async () => {
    try {
      ...
      const {
        data: driversData,
        error
      } = await typedSupabase.from('profiles').select('*').eq('role', 'chauffeur').order('created_at', {
        ascending: false
      });
  ...
  const fetchDriverConfig = async (driverId: string) => {
    try {
      const {
        data,
        error
      } = await typedSupabase.from('drivers_config').select('*').eq('id', driverId).single();
  ...
  const handleDeleteDocument = async (driverId: string, documentType: string, fieldName: string) => {
    ...
    try {
      ...
      if (path) {
        const {
          error: storageError
        } = await typedSupabase.storage.from('driver.doc.config').remove([path]);
      ...
      const {
        error: dbError
      } = await typedSupabase.from('drivers_config').update({
        [fieldName]: null
      }).eq('id', driverId);
  ...
  const getDocumentUrl = (path: string | null): string | null => {
    ...
    try {
      const {
        data
      } = typedSupabase.storage.from('driver.doc.config').getPublicUrl(path);
  ...
  const getDriverDocuments = (config: DriverConfig | null): DriverDocument[] => {
    ...
    return [{
      type: 'license',
      path: config.license_document_path,
      label: 'Permis de conduire'
    }, {
      type: 'id',
      path: config.id_document_path,
      label: 'Pièce d\'identité'
    }, {
      type: 'kbis',
      path: config.kbis_document_path,
      label: 'KBIS'
    }, {
      type: 'vigilance',
      path: config.vigilance_document_path,
  ...
  return <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {loading ? <div className="flex justify-center py-10">
            ...
            </div> : filteredDrivers.length === 0 ? <div className="text-center py-10 text-neutral-500">
          ...
          {selectedDriver && <div className="space-y-6">
              ...
              <div className="border-t pt-4">
                ...
                <Table>
                  ...
                  <TableBody>
                    {getDriverDocuments(driverConfig).map(doc => <TableRow key={doc.type}>
                        <TableCell>{doc.label}</TableCell>
                        <TableCell>
                          {doc.path ? <span className="text-green-600 font-medium">Téléchargé</span> : <span className="text-amber-600">Non fourni</span>}
                        </TableCell>
                        <TableCell className="text-right">
                          {doc.path && <>
                              <Button variant="ghost" size="sm" onClick={() => openDocumentPreview(doc.path, doc.label)} className="h-8 w-8 p-0">
                                <Eye className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="sm" onClick={() => handleDeleteDocument(selectedDriver.id, doc.label, `${doc.type}_document_path`)} className="h-8 w-8 p-0 text-red-500 hover:text-red-600 hover:bg-red-50">
                                <X className="h-4 w-4" />
                              </Button>
                            </>}
                        </TableCell>
      ...
      <Dialog open={!!previewDocument} onOpenChange={open => !open && setPreviewDocument(null)}>
        <DialogContent className="sm:max-w-3xl">
          ...
          <div className="mt-2">
            {previewDocument && <div className="flex justify-center">
                {previewDocument.url.toLowerCase().endsWith('.pdf') ? <iframe src={previewDocument.url} className="w-full h-[70vh]" title="Document Preview" /> : <img src={previewDocument.url} alt="Document Preview" className="max-h-[70vh] object-contain" />}
              </div>}
          </div>
        </DialogContent>
      </Dialog>
