    ...
    const fetchClients = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('profiles').select('id, full_name, company_name, email').eq('role', 'client');
  ...
  const fetchMissions = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('missions').select('*').in('status', ['livre', 'termine']).order('created_at', {
        ascending: false
      });
      if (error) throw error;
  ...
  return <div className="space-y-6 px-[15px]">
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <Input type="search" placeholder="Rechercher une facture..." className="pl-8" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
        ...
        {statusFilter !== 'all' && <Button variant="ghost" size="sm" onClick={clearFilter} className="h-9 w-9 p-0">
            <X size={16} />
          </Button>}
      </div>
      ...
      <Card>
        <CardHeader className="pb-3 md:pb-4">
          <CardTitle className="text-center md:text-left text-xl">Missions facturées</CardTitle>
        </CardHeader>
        <CardContent className="px-3 md:px-6">
          <div className="overflow-x-auto -mx-3 md:mx-0">
            <InvoicesTable missions={filteredMissions} clientsData={clientsData} isLoading={loading} userRole="admin" onMissionStatusUpdate={fetchMissions} layout={isMobile ? "stacked" : "table"} />
          </div>
        </CardContent>
      </Card>
    </div>;
