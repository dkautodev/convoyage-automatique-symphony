...
interface AdminMissionsFiltersProps {
  activeTab: AdminMissionTab;
  setActiveTab: (tab: AdminMissionTab) => void;
  missionCountsByStatus: Record<MissionStatus, number>;
  totalMissions: number;
}

const AdminMissionsFilters: React.FC<AdminMissionsFiltersProps> = ({
  activeTab,
  setActiveTab,
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-full bg-background border shadow-lg z-50">
            {statusTabs.map((tab) => (
              <DropdownMenuItem
                key={tab.id}
                onSelect={() => setActiveTab(tab.id as AdminMissionTab)}
  ...
  return (
    <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as AdminMissionTab)}>
